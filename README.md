...
## 最新版本 v1.0.18-secure-ai-replay-drawer

- 新增防作弊資料拆分設計文件與牌局健康檢查提醒，規劃 public / privateHands / reveal / audit 路徑。
- AI 叫牌升級為「自然制第一版」：含 HCP、五張高花、1NT 15–17、強 2♣、弱二、阻擊、支持同伴、競叫與 Double 判斷。
- 叫牌教學理由強化：教練建議會說明為什麼建議該叫品，叫牌按鈕也有提示文字。
- 牌局回放升級為逐步回放：可上一步 / 下一步 / 自動播放，並顯示該步桌面牌、目前最大牌與各家剩餘手牌。
- 手機版右側紀錄區改成可收合抽屜與分頁，可切換工具、叫牌、檢討、出牌與紀錄。
...
> 注意：v1.0.18 已加入防作弊資料拆分設計與規則範例；目前主程式仍保留相容舊版的房主瀏覽器主持架構。若要正式競賽級防作弊，請依 `ANTI_CHEAT_FIREBASE_DESIGN.md` 改成 public/privateHands 訂閱，並以 Cloud Functions 或受限房主流程驗證牌局動作。
