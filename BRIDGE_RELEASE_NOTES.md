## bridge-v1.0.24.8-ai-bidding-guardrails

- 修正 AI 叫牌會在自己已是最高叫品時，被對手 Double 後一路把同一門花色抬高的問題。
- AI 不再反覆無意義 Double；Double 改成需要符合低階 Takeout Double 或高階強防守條件。
- 同伴合約被 Double 時，AI 會優先 Pass / Redouble / 簡單加叫，不再盲目跳高。
- 回應同伴低花時增加安全上限，三階以上只有明顯強牌才繼續加叫。
- 競叫更保守：二階以下需要足夠牌力與五張長門，三階競叫需要六張與牌質。

## bridge-v1.0.24.7-context-focused-ui

- 叫牌階段改成專注介面：左側只保留階段、叫牌摘要與叫牌操作；右側只顯示叫牌紀錄表。
- 出牌階段改成專注介面：左側顯示合約、王牌、墩數與等待狀態；右側只保留每墩出牌紀錄與牌局檢討。
- 合約面板依階段改變內容，減少節奏、維護、記錄等非當前操作資訊干擾。
- 手機版紀錄抽屜會依叫牌 / 出牌階段自動切換到最相關分頁。


## bridge-v1.0.24.6-win-lose-animation

- 本局結果視窗改為依照玩家座位顯示「勝利 / 失敗 / 平手」。
- 若是觀戰者，結果視窗會顯示本副勝方。
- 勝利時顯示金色彈出與彩帶動畫；失敗時顯示紅色提示與震動式動畫；平手 / 觀戰使用中性動畫。
- 勝敗判定以本副分數變動的 NS / EW 得分方為準，防守方擊倒合約也會正確顯示防守方勝利。
